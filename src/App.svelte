<script lang="ts">
  ...
  let prefersLightChangeHandler: ((event: MediaQueryListEvent) => void) | null =
    null;
  ...
  $: designStormIcon =
    theme === "light" ? designStormDarkIcon : designStormLightIcon;
  ...
  function handleNoaaTabKeydown(event: KeyboardEvent, index: number) {
    ...
    const tabButtons = noaaTablistEl
      ? (Array.from(
          noaaTablistEl.querySelectorAll<HTMLButtonElement>(
            'button[role="tab"]',
          ),
  ...
  let selectedCurveDuration: (typeof STANDARD_DURATION_HOURS)[number] | null =
    null;
  ...
  $: {
    ...
    const safeDuration = Number.isFinite(numericDuration)
      ? numericDuration
      : DEFAULT_DURATION_HOURS;
    resolvedDistribution = getBestScsDistribution(
      $distribution,
      safeDuration,
      $durationMode,
    );
  ...
  $: {
    const lines: string[] = [];
    const numericDuration = Number($selectedDurationHr);
    const durationHr = Number.isFinite(numericDuration)
      ? numericDuration
      : null;
    ...
    const tableDurationText = formatDurationText(
      selectedDurationLabel,
      durationHr,
    );
    const depthValue = Number.isFinite($selectedDepth)
      ? Number($selectedDepth)
      : null;
    const depthText = depthValue != null ? `${depthValue.toFixed(2)} in` : null;
    const ariValue = Number.isFinite($selectedAri)
      ? Number($selectedAri)
      : null;
    ...
    if ($distribution === "user") {
      if (customCurveLines.length >= 2) {
        ...
        distributionLine =
          "Rainfall pattern falls back to a straight-line ramp because no custom CSV points are loaded.";
      }
    } else if (resolvedDistribution !== $distribution) {
      const modeLabel =
        $durationMode === "standard" ? "preset" : "custom duration";
    ...
    if (depthText && ariText) {
      if (table && selectedDurationLabel) {
        ...
        const hasExactDepth =
          Number.isFinite(tableDepth) &&
          Math.abs(Number(tableDepth) - depthValue!) < 0.005;
    ...
    if (lastStorm) {
      ...
      const sampleCount = Array.isArray(lastStorm.timeMin)
        ? lastStorm.timeMin.length
        : null;
  ...
  function attachNoaaScrollListener() {
    ...
    if (noaaTableScrollEl) {
      noaaTableScrollEl.addEventListener("scroll", handleNoaaScroll, {
        passive: true,
      });
  ...
  function updateNoaaStickyOffset() {
    ...
    noaaTableScrollEl.style.setProperty(
      "--noaa-scroll-left",
      `${noaaTableScrollEl.scrollLeft}px`,
    );
  }
  ...
  function reduceTickEntries<T>(entries: T[], maxCount: number) {
    ...
    return entries.filter(
      (_, index) => index % step === 0 || index === entries.length - 1,
    );
  }
    ...
    try {
      ...
      if (!hadTable) {
        ...
        if (initialScrollIndex != null) {
          ...
          pendingNoaaScrollIndex =
            defaultDurationIndex >= 0 ? defaultDurationIndex : null;
        }
      } else {
      ...
      if (!hadTable && defaultDurationLabel) {
        ...
        selectedDurationLabel =
          defaultDurationLabel ?? parsed.rows[0]?.label ?? null;
      }
  ...
  function matchesStandardDurationHours(hours: number) {
    return STANDARD_DURATION_HOURS.some(
      (standard) => Math.abs(hours - standard) < 1e-6,
    );
  }

  function hasComparisonCurveForDuration(
  ...
  function drawIsoPlot() {
    ...
    const filteredDurationEntries = reduceTickEntries(
      durationEntries,
      maxXTicks,
    );
    ...
    const colorbar: Partial<ColorBar> = {
      title: {
        text: "Depth (in)",
        font: {
          color: chartTheme.isoColorbarText,
      ...
      tickfont: {
        color: chartTheme.isoColorbarTick,
    ...
    if (table && selectedDurationLabel) {
      ...
      const highlightAri = ariEntries.find(
    ...
    if (
      ...
      const depthText = hasDepth
        ? `<br>Depth: ${$selectedDepth.toFixed(3)} in`
        : "";
      stormTrace = {
        type: "scatter",
        mode: "markers",
  ...
  function drawNoaa3dPlot() {
    ...
    const textMatrix = noaaAriEntries.map((ariEntry, rowIdx) =>
      noaaDurationEntries.map((durationEntry, colIdx) => {
        ...
        const depthText = Number.isFinite(depth as number)
          ? `${(depth as number).toFixed(2)} in`
          : "—";
        const intensityText = Number.isFinite(intensity as number)
          ? `${(intensity as number).toFixed(2)} in/hr`
          : "—";
    ...
    const colorbar: Partial<ColorBar> = {
      title: {
        text: "Intensity (in/hr)",
        font: { color: chartTheme.isoColorbarText },
      },
      tickcolor: chartTheme.isoColorbarTick,
    ...
    const selectedNoaaTrace = (() => {
      ...
      const ariEntry = noaaAriEntries.find(
        (entry) => entry.key === String($selectedAri),
      );
      ...
      if (
        !Number.isFinite(intensity as number) ||
    ...
    const layout: Partial<Layout> = {
      ...
      scene: {
        ...
        xaxis: {
          ...
          title: {
            text: "Duration (hr)",
            font: { color: chartTheme.sceneText },
          },
  ...
  function drawNoaaIntensityPlot() {
    ...
    const traces: Data[] = ariEntries.map((ariEntry, rowIdx) => {
      ...
      const color =
        chartTheme.idfPalette[rowIdx % chartTheme.idfPalette.length];
    ...
    if (table && selectedDurationLabel) {
      ...
      const ariIndex = ariEntries.findIndex(
        (entry) => entry.key === String($selectedAri),
      );
      ...
      if (
        ...
        const highlightDepthValue = Number.isFinite(highlightDepth as number)
          ...
          : null;
        const depthSegment = Number.isFinite(highlightDepth as number)
        ...
        highlightTrace = {
          ...
          showlegend: false,
          customdata: [
            [durationEntry.label, ariEntry.key, highlightDepthValue],
          ],
          hovertemplate:
            `Duration: ${durationEntry.label} (${durationEntry.hr.toFixed(2)} hr)` +
  ...
  const handleNoaaIntensityPlotClick = (event: any) => {
    ...
    const curveNumber =
      typeof point.curveNumber === "number" ? point.curveNumber : null;
    const pointIndex =
      typeof point.pointIndex === "number" ? point.pointIndex : null;
  ...
  function recalcFromDepthOrDuration() {
    ...
    const result = interpolateAriForDuration(
      table,
      durationHr,
      $selectedDepth,
      {
  ...
  $: if ($durationMode === "standard") {
    ...
    const needsNormalization =
      !Number.isFinite(numericDuration) || numericDuration !== presetHours;
  ...
  function findHighlightBetweenAris(
    row: NoaaRow,
    aris: string[],
    targetAri: number,
  ) {
  ...
  function recalcFromAri() {
    ...
    if (result) {
      ...
      interpolatedCells =
        result.highlight ??
        findHighlightBetweenAris(row, table.aris, $selectedAri) ??
        [];
      isExtrapolating = result.extrapolated;
    } else {
      interpolatedCells = [];
  ...
  function doCsv() {
    ...
    const startDate = $startISO ? new Date($startISO) : null;
    const start =
      startDate && !Number.isNaN(startDate.getTime()) ? $startISO : undefined;
  ...
  function updateSelectedCurveData() {
    selectedCurveData =
      selectedCurveDistribution != null
        ? (comparisonCurves.find(
            (curve) => curve.distribution === selectedCurveDistribution,
          ) ?? null)
        : null;
  }

  function setComparisonDuration(
    duration: (typeof STANDARD_DURATION_HOURS)[number],
  ) {
  ...
  async function refreshComparisonCurves() {
    ...
    if (
      ...
      !matchesStandardDurationHours(selectedCurveDuration)
    ) {
      ...
      const preferred = matchesStandardDurationHours(currentDurationHr)
        ? (nearestStandardDuration(
            currentDurationHr,
          ) as (typeof STANDARD_DURATION_HOURS)[number])
    ...
    if (key === lastCurveParamsKey && comparisonCurves.length) {
      if (
        ...
        !comparisonCurves.some(
          (curve) => curve.distribution === selectedCurveDistribution,
        )
      ) {
        const fallback =
          comparisonCurves.find(
            (curve) => curve.distribution === $distribution,
          ) ?? comparisonCurves[0];
    ...
    try {
      ...
      comparisonCurves = availableMembers.map((member) => {
        ...
        const totalDepth =
          result.cumulativeIn[result.cumulativeIn.length - 1] || 1;
  ...
  const handleCurvePlotClick = (event: any) => {
    ...
    const dist =
      Array.isArray(customData) && typeof index === "number"
        ? customData[index]
        : undefined;
  ...
  function drawComparisonCurves() {
    ...
    const traces = comparisonCurves.map(
      (curve) =>
        ({
          ...
          name: curve.label,
          line: {
            width: curve.distribution === selectedCurveDistribution ? 4 : 2,
          },
    ...
    const maxDuration =
      selectedCurveDuration ??
      Math.max(
        ...comparisonCurves.map(
          (curve) => curve.timeHr[curve.timeHr.length - 1] ?? 0,
        ),
      );
  ...
  onMount(() => {
    ...
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          console.warn(
            "Unable to access geolocation; using continental US view.",
            error,
          );
          setMapViewToContinentalUs();
        },
...
<div class="page">
  ...
  {/if}
  <header class="panel header animate-slide-in-up">
  ...
  <div class="layout">
    <section class="column column--data animate-slide-in-up delay-100">
      <div class="panel">
        ...
        <div class="options-row">
          ...
          >
            {#if isLoadingNoaa}
              <span class="noaa-refresh-button__spinner" aria-hidden="true"
              ></span>
        ...
        {#if $tableStore}
            ...
            <div class="noaa-table panel">
              ...
              >
                <div class="table-header__ari">
                  <span class="ari-label"
                    >Average Recurrence Interval (years)</span
                  >
                </div>
                {#each durationEntriesForTable as entry}
                  {@const isSelectable =
                    isCustomDurationMode ||
                    durationLabelIsStandard(entry.label)}
                  <div
                    class="table-header__duration"
                  ...
                  >
                    <button
                      ...
                      on:click={() =>
                        pickCell(entry.label, String($selectedAri))}
                    >
                      {entry.label}
                    </button>
                  </div>
                ...
                {#each aris as ariKey}
                  ...
                  >
                    ...
                    {#each durationEntriesForTable as entry}
                      ...
                      {@const depthValue = Number.isFinite(rawDepth)
                        ? Number(rawDepth)
                        : null}
                      {@const depthText =
                        depthValue != null ? depthValue.toFixed(3) : ""}
                      <button
                        type="button"
                        class="table-button cell"
                        class:selected={selectedDurationLabel === entry.label &&
                          String($selectedAri) === ariKey}
                        class:column-active={selectedDurationLabel ===
                          entry.label}
                        class:interpolated={interpolatedCellKeys.has(
                          `${entry.label}::${ariKey}`,
                        )}
                        disabled={!isSelectable}
                        data-ari={ariKey}
          ...
          <div class="small">
            Tip: Click a table cell to apply the depth, duration, and Average
            Recurrence Interval to the storm parameters.
          </div>
        {/if}
      </div>
            ...
            {#if activeNoaaVisual === "isoLines"}
              ...
              >
                <div class="iso-plot-container">
                  ...
                  ></div>
                  {#if isoPlotIsRendering}
                    <div class="plot-loading-overlay" transition:fade>
                      <LoadingSpinner size={40} />
                    </div>
                  {/if}
                  {#if !$tableStore}
                    <div class="iso-plot-empty">
                      Load NOAA data to view the depth iso-line preview.
                    </div>
                  {/if}
                </div>
              </div>
            ...
            {#if activeNoaaVisual === "rdi3d"}
              ...
              >
                <div class="iso-plot-container">
                  ...
                  {#if !$tableStore}
                    <div class="iso-plot-empty">
                      Load NOAA data to view the rainfall depth-intensity
                      surface.
                    </div>
                  {/if}
                </div>
            ...
            {#if activeNoaaVisual === "intensity"}
              ...
              >
                <div class="iso-plot-container">
                  ...
                  ></div>
                  {#if noaaIntensityPlotIsRendering}
                    <div class="plot-loading-overlay" transition:fade>
                      <LoadingSpinner size={40} />
                    </div>
                  {/if}
                  {#if !$tableStore || !noaaIntensityZ.length}
                    <div class="iso-plot-empty">
                      Load NOAA data to view the intensity-duration-frequency
                      curves.
                    </div>
                  {/if}
                </div>
    ...
    <section class="column column--controls">
      <div class="panel animate-slide-in-up delay-200">
          ...
          <div class="storm-processing-slot" aria-live="polite">
            {#if isStormProcessing}
              <div class="storm-processing-indicator" role="status">
                <span
                  class="storm-processing-indicator__spinner"
                  aria-hidden="true"
                ></span>
                <span class="storm-processing-indicator__text"
                  >Processing storm…</span
                >
              </div>
            {/if}
            {#if stormProcessingRainVisible}
        ...
        <div class="storm-form">
          <div class="storm-form__header">
            <div class="storm-card storm-card--distribution">
                ...
                <div class="custom-curve-preview" aria-live="polite">
                  ...
                  {#if customCurveLines.length > 3}
                    <div class="custom-curve-preview-more">
                      +{customCurveLines.length - 3} additional row{customCurveLines.length -
                        3 ===
                      1
                        ? ""
                        : "s"}
              ...
              <div class="field-hint">
                Note: Huff distributions are approximated using Beta
                distributions.
              </div>
            </div>
            <div class="storm-card storm-card--mode">
              <div class="mode-header animate-fade-in">
              ...
              {#if $durationMode === "custom"}
                <div
                  class="mode-note field-hint field-hint--danger mode-warning"
                  role="alert"
                >
                  <strong>Warning:</strong> Custom durations stretch NRCS curves
                  beyond NOAA's validated 6-, 12-, and 24-hour patterns. Expect smoother
                  peaks than real short storms and muted multi-day variability—verify
                  with local IDF/Huff data or your own hyetograph before relying
                  on results, and review Help → Atlas 14 Distribution Limits for
                  guidance.
                </div>
              {:else}
                <p class="mode-note">
                  Quickly select 6-, 12-, or 24-hour durations using the presets
                  below.
                </p>
              {/if}
              <p class="mode-note mode-note--computation">
                {#if $computationMode === "fast"}
                  Fast mode downsamples the storm to {MAX_FAST_SAMPLES.toLocaleString()}
                  evenly spaced timesteps for quicker updates. Switch back to Precise
                  for the full-resolution dataset.
                {:else}
                  Precise mode follows every timestep for maximum fidelity. Use
                  Fast (approx.) if storms take too long to compute.
                {/if}
              </p>
            </div>
          </div>
          ...
          <div class="storm-form__inputs">
            ...
            {#if isExtrapolating}
              <p
                class="field-hint field-hint--warning storm-form__extrapolation-note"
                role="status"
              >
                <strong>Extrapolation in effect.</strong> Values extend beyond the
                NOAA table and are estimated using the nearest trend.
              </p>
            {/if}
      ...
      <div class="panel storm-summary-panel">
        ...
        {:else}
          <p class="empty">
            Adjust the parameters to see how the storm will be constructed.
          </p>
        {/if}
      </div>
    </section>

    <section class="column column--visuals animate-slide-in-up delay-300">
      ...
      <div class="panel plot">
        <button
          ...
          class="plot-download"
          on:click={() =>
            downloadPlot(plotDiv1, "designstorm-hyetograph-intensity")}
        ...
        {#if hyetographSmoothingActive}
          <p class="plot-note field-hint field-hint--info" role="status">
            Fast (approx.) mode hit the {MAX_FAST_SAMPLES.toLocaleString()} sample
            cap, smoothing the hyetograph to ~{hyetographSmoothingStepLabel}-minute
            bars. Switch to Precise to restore the requested
            {hyetographRequestedStepLabel ||
              formatMinutesLabel($timestepMin)}-minute timestep.
          </p>
        {/if}
      </div>
      <div class="panel plot">
        <button
          ...
          class="plot-download"
          on:click={() =>
            downloadPlot(plotDiv2, "designstorm-incremental-volume")}
      ...
      <div class="panel plot">
        <button
          ...
          class="plot-download"
          on:click={() =>
            downloadPlot(plotDiv3, "designstorm-cumulative-mass-curve")}
      ...
      <div class="panel results">
                ...
                {#each tableRows as row}
                  <tr>
                    <td class="left" data-label={timeColumnLabel}
                      >{row.time.toFixed(2)}</td
                    >
                    <td data-label="Intensity (in/hr)"
                      >{row.intensity.toFixed(5)}</td
                    >
                    <td data-label="Incremental (in)"
                      >{row.incremental.toFixed(5)}</td
                    >
                    <td data-label="Cumulative (in)"
                      >{row.cumulative.toFixed(5)}</td
                    >
                    {#if hasTimestamp}
        ...
        {:else}
          <p class="empty">
            Adjust the parameters to see the time series table.
          </p>
        {/if}
      </div>
    </section>
  ...
  >
    ...
    >
      <div class="modal-content">
        ...
        <p class="small">
          Provide normalized cumulative pairs in CSV format (time fraction,
          cumulative depth fraction).
        </p>
        <label class="sr-only" for="custom-curve-textarea"
          >Custom curve CSV input</label
        >
        <textarea
        ...
        ></textarea>
        <div class="field-hint">
          The curve will be trimmed, normalized, and interpolated to match the
  ...
  >
    ...
    >
      <div class="modal-content">
        ...
        <p class="small">
          Compare cumulative rain fraction over time across available {comparisonGroupLabel ||
            "selected"}
          distributions. Choose a standard duration to update the plot, and click
          a curve to inspect its normalized values.
        </p>
        <div class="curve-controls">
          <span class="curve-group-label"
            >{comparisonGroupLabel || "Distributions"}</span
          >
          <div
            class="curve-duration-toggle"
            role="group"
            aria-label="Select storm duration"
          >
        ...
        <div class="curve-plot-wrapper">
          ...
          <button
            ...
            class="plot-download"
            on:click={() =>
              downloadPlot(curvePlotDiv, "designstorm-distribution-comparison")}
        ...
        {:else}
          <p class="empty">
            Unable to generate comparison curves. Adjust parameters and try
            again.
          </p>
        {/if}
      </div>
...
<style>
  ...
  .custom-curve-preview pre {
    margin: 0;
    font-family: "JetBrains Mono", "Fira Mono", "SFMono-Regular", Menlo, Monaco,
