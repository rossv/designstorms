<script lang="ts">
  ...
  const NOAA_ARI_COLUMN_WIDTH = 'clamp(160px, 18vw, 220px)'
  const NOAA_DURATION_COLUMN_WIDTH = 'minmax(88px, 1fr)'
        ...
        {#if $tableStore}
          <div class="noaa-table-scroll" bind:this={noaaTableScrollEl}>
            <div class="noaa-table panel">
              <div
                class="table-header"
                style={`grid-template-columns: ${NOAA_ARI_COLUMN_WIDTH} repeat(${durationEntriesForTable.length}, ${NOAA_DURATION_COLUMN_WIDTH});`}
              >
                ...
                {#each durationEntriesForTable as entry}
                  <div
                    ...
                    class:column-disabled={!durationIsSelectable(entry.label)}
                  >
                    <button
                      type="button"
                      ...
                      disabled={!durationIsSelectable(entry.label)}
...
<style>
  ...
  .table-header__duration.column-disabled {
    opacity: 0.55;
  }
  ...
  .table-button.cell.column-disabled {
    opacity: 0.45;
  }

  .table-button.cell.column-disabled:hover {
    background: transparent;
    color: inherit;
  }
